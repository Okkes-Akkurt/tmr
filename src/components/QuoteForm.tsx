...
export default function QuoteForm({ locale, d, whatsapp, email, defaultService = '' }: Props) {
	...
	useEffect(() => {
		const h = (e: Event) => {
			...
			setTimeout(() => document.getElementById('q_name')?.focus({ preventScroll: true }), 500);
		};
	...
	const waText = () => {
		...
		const lines = [
			d['wa.quote'],
			`${d['wa.name']}: ${val('name')}`,
			`${d['wa.phone']}: ${val('phone')}`,
			val('company') && `${d['wa.company']}: ${val('company')}`,
			`${d['wa.service']}: ${svc}`,
			val('location') && `${d['wa.loc']}: ${val('location')}`,
			val('message') && `\n${val('message')}`,
		].filter(Boolean);
	...
	const errNames: Record<string, string> = {
		name: d['f.errName'],
		phone: d['f.errPhone'],
		email: d['f.errEmail'],
		company: d['f.errCompany'],
		service: d['f.errService'],
		location: d['f.errLoc'],
		message: d['f.errMsg'],
		kvkk: d['f.errKvkk'],
	};
	...
	return (
		<form
			...
			<div className='fgrid'>
				<div className='f'>
					<label htmlFor='q_name'>{d['f.name']}</label>
					<input
						id='q_name'
						name='name'
						required
				...
				<div className='f has-tip'>
					<label htmlFor='q_phone'>{d['f.phone']}</label>
					<input
						id='q_phone'
						name='phone'
						type='tel'
						...
						autoComplete='tel'
						inputMode='tel'
						placeholder={d['f.phoneph']}
						aria-describedby='q_phone_hint'
				...
				<div className='f'>
					<label htmlFor='q_email'>{d['f.email']}</label>
					<input
						id='q_email'
						name='email'
						type='email'
				...
				<div className='f'>
					<label htmlFor='q_company'>{d['f.company']}</label>
					<input
						id='q_company'
						name='company'
						required
				...
				<div className='f'>
					<label htmlFor='q_service'>{d['f.service']}</label>
					<select
						id='q_service'
						name='service'
						required
						...
						{GROUPS.map((g) => (
							<optgroup
								key={g.id}
								label={g[locale].t}>
								{[...g.ids, ...g.extraIds].map((id) => (
									<option
										key={id}
										value={id}>
										{serviceById(id)![locale].t}
									</option>
								))}
							</optgroup>
						))}
				...
				<div className='f'>
					<label htmlFor='q_loc'>{d['f.loc']}</label>
					<input
						id='q_loc'
						name='location'
						required
						placeholder={d['f.locph']}
						aria-invalid={invalid('location')}
					/>
				</div>
				<div className='f full'>
					<label htmlFor='q_msg'>{d['f.msg']}</label>
					<textarea
						id='q_msg'
						name='message'
						required
			...
			<div className='form-foot'>
				<button
					...
					type='submit'
					disabled={pending}>
					{pending ? d['f.sending'] : d['f.submit']}
				</button>
				{email && (
					<span className='note'>
						{d['f.note1']} <a href={`mailto:${email}`}>{d['f.note2']}</a> {d['f.note3']}
					</span>
				)}
			</div>
			<div
				...
				{state.status === 'ok' && (
					<div className='msg'>
						<strong>{d['f.ok1']}</strong> {state.phone ? fmt(d['f.ok2'], { phone: state.phone }) : ''}{' '}
						<a
							href={waText()}
							target='_blank'
							rel='noopener'>
				...
				{state.status === 'error' && state.code === 'invalid' && (
					<div className='msg err'>
						{d['f.errPre']}
						{(state.fields || []).map((f) => errNames[f]).join(', ')}.
					</div>
				)}
				{state.status === 'error' && state.code !== 'invalid' && (
					<div className='msg err'>
						{state.code === 'rate' ? d['f.errRate'] : d['f.errGeneric']}{' '}
						<a
							href={waText()}
							target='_blank'
							rel='noopener'>
